...
! Evaluates B at the Earth's surface on the defined (theta,phi) grid
   ...
   IMPLICIT NONE
   INTEGER :: NTHETA_GRID, NPHI_GRID, LMAX, LMAX_B_OBS
   REAL( KIND = LONG_REAL) :: GAUSS(1:), B_R_SURF(1:,0:),  B_T_SURF(1:,0:), B_P_SURF(1:,0:), ALF(1:,1:), DALF(1:,1:), ONE_DIV_SINTHETA(1:)
   TYPE (HARMONIC_STRUCTURE) :: HARMONICS(1:)
   ...
   B_R_SURF(:,:) = 0.0_LONG_REAL
   B_T_SURF(:,:) = 0.0_LONG_REAL
   B_P_SURF(:,:) = 0.0_LONG_REAL
...
! caclulate horizontal laplacian of function B_r at the CMB from the gauss coefficients
   IMPLICIT NONE
   INTEGER :: NTHETA_GRID, NPHI_GRID, LMAX, LMAX_B_OBS
   REAL( KIND = LONG_REAL) :: GAUSS(1:), NABLA2_B_R(1:,0:), ALF(1:,1:)
   TYPE (HARMONIC_STRUCTURE) :: HARMONICS(1:)
