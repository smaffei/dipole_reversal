...
REAL( KIND = EXTRA_LONG_REAL) :: COSTHETA_LOC, SINTHETA_LOC, PHI_DEP, DERV_PHI_DEP, B_DOT_LOCATION(3), F_LOC, D_LOC, I_LOC, MAX_INTENSITY_RATE, NORM_FACTOR, KE, TOTAL_RMS, VALUE(1:4), DIPOLE_TILT_RATE, NORM_G
REAL( KIND = EXTRA_LONG_REAL) :: LOCATIONS(1), X, P, Y
...
DO
  ...
  IF ( FLAG_U_INIT.EQ.1 .OR. N_IT.GT.1) THEN
	...
	ENDIF
      ...
      ! ***********************************************
      ! ***********************************************
        I_LOC = TAN( -B_LOCATION(1)/SQRT(B_LOCATION(2)**2 + B_LOCATION(3)**2) )
	  ...
	  ! OPTIMIZE VGP LATITUDE
	    ...
	    DD_LOC_DT = (B_DOT_LOCATION(2) * B_LOCATION(3) - B_DOT_LOCATION(3) * B_LOCATION(2)) / ((B_LOCATION(2) * B_LOCATION(2))&
	    + (B_LOCATION(3) * B_LOCATION(3)))
	    ! GREAT CIRLCE DISTANCE TO VGP
	    P = ATAN( 2/TAN(I_LOC) )
	    ! DP/DT
	    Y = 2/ATAN(I_LOC)
	    DP_DT = ( -2*DI_LOC_DT/ ( SIN(I_LOC) )**2 ) / (1+Y**2)
	    ! AUXILLIARY QUANTITY
	    X = COSTHETA_LOC*COS(P) + SINTHETA_LOC*SIN(P)*COS(D_LOC)
	    ! finally, G
	    G( HARMONIC, HARMONIC_TYPE ) = ( -COSTHETA_LOC*SIN(P)*DP_DT + SINTHETA_LOC*COS(P)*DP_DT*COS(D_LOC) - SINTHETA_LOC*SIN(P)*SIN(D_LOC)*DD_LOC_DT ) / SQRT(1-X**2)
